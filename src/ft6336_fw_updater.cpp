...
#define NEW_FIRMWARE_VERSION 18
...
static void ft6x36_fw_upgrade(m5gfx::DataWrapper* data, uint32_t data_len)
{
  int i;
...
void ft6336_fw_updater(m5gfx::DataWrapper* data, size_t length)
{
  uint8_t wbuf, rbuf;
  ...
  {
    ft6x36_fw_upgrade(data, length);
  }
}

void ft6336_fw_updater(const uint8_t* array, size_t len)
{
  {
    M5.Display.print("fw update from: internal");
    M5.Display.println("please wait about 5 second...");
    m5gfx::PointerWrapper data;
    data.set(array, len);
    ft6336_fw_updater(&data, len);
    data.close();
  }
}
